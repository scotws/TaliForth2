...
Apart from that, the `docs` folder looks messier than it is -- what you are
probably looking for are the `manual.html`, `manual.pdf` and possibly the
`WORDLIST.md` files. There are also listing files here generated by the Ophis
assembler.

The `TODO.txt` list is not always up to date, see the list of issues on the
GitHub home of Tali Forth 2. It will probably be frozen in a future version.
