...
zero-terminated. By default, the strings of Tali Forth are lower case, but case
is respected for words the user defines, so `quarian` is a different word than
`QUARIAN`. 

NOTE: Currently, upper case versions of the ANS Forth words are not recognized.
