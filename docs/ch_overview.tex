...
in the Forth Kernel}} by Brad Rodriguez\index{Rodriguez, Brad}}
...
register, the accumulator A\index{accumulator}, and two secondary registers
X\index{X register} and Y\index{Y register}. All are 8-bit wide, which is half a
cell. There are 255 bytes that are more easily addressable on the Zero
Page\index{Zero Page}. A single hardware stack is used for subroutine jumps. The
address bus\index{address bus} is 16 bits wide for a maximum of 64 KiB of
RAM\index{RAM} and \index{ROM} -- for a simple setup, we should assume 32 KiB of
each. 
