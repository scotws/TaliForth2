...
\begin{itemize}
        ...
        \item \texttt{0branch}\index{0branch@\texttt{0branch}} \texttt{( f -- )} 
        ...
        \item \texttt{0}\index{0@\texttt{0}} \texttt{( -- 0 )} 
                Push the number 0 on the Data Stack.

        \item \texttt{1}\index{1@\texttt{1}} \texttt{( -- 1 )} 
                Push the number 1 on the Data Stack.

        \item \texttt{2}\index{2@\texttt{2}} \texttt{( -- 2 )} 
                Push the number 2 on the Data Stack.

        \item \texttt{always-native}\index{always-native@\texttt{always-native}}\texttt{( -- )} 
                Mark latest word so that it is always natively compiled. 
   
        \item \texttt{bell}\index{bell@\texttt{bell}} \texttt{( -- )} 
                Ring the terminal bell (print ASCII\index{ASCII} 07)

        \item \texttt{branch}\index{branch@\texttt{branch}}\texttt{( -- )} 
        ...
        \item \texttt{compile-only}\index{compile-only@\texttt{compile-only}}
                \texttt{( -- )} Mark latest word as compile only.

        \item \texttt{digit?}\index{digit?@\texttt{digit?}} 
                \texttt{( char -- u f | char f )} If character is a digit,
                convert and set a success flag, otherwise return the character
                and a failure flag.

        \item \texttt{input}\index{input@\texttt{input}} \texttt{( -- addr )} 
        ...
        \item \texttt{int>name}\index{int>name@\texttt{int>name}} \texttt{( xt -- nt )} 
                Given the execution token\index{execution token} of a
                word, return the name token.\index{name token}. 
                
        \item \texttt{latestnt}\index{latestnt@\texttt{latestnt}} \texttt{( -- nt )} 
        ...
        \item \texttt{nc-limit}\index{nc-limit@\texttt{nc-limit}}\texttt{( -- addr )} 
                Return the address where the threshold value for native
                compiling\index{native compiling} is kept.  

        \item \texttt{never-native}\index{never-native@\texttt{never-native}} \texttt{( -- )} 
                Mark most recent word so it is never natively compiled.

        \item \texttt{number}\index{number@\texttt{number}} 
        ...
        \item \texttt{output}\index{output@\texttt{output}} \texttt{( -- addr )} 
