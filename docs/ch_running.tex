...
Take for instance \texttt{drop}\index{drop@\texttt{drop}}, which in its naive
form is simply
...
underflow\index{underflow}, so the actual assembler code is
...
compiling}\index{compiling, native}. The system variable \texttt{nc-limit} sets
...
Tali has to use the upper cell of a double-celled\index{double cell} number to
correctly report the result, while Gforth doesn't. If the conversion from double
to single is only via a \texttt{drop} instruction, this will produce different
results.
